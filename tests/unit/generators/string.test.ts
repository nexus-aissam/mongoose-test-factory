...
describe("MongooseStringGenerator", () => {
  ...
  describe("validation", () => {
    ...
    it("should validate with length constraints", () => {
      ...
      // Valid length
      expect(generator.validate(value, { minLength: 3, maxLength: 10 })).toBe(
        true
      );
    ...
    it("should validate with regex constraints", () => {
      ...
      expect(generator.validate(nonEmailValue, { match: emailRegex })).toBe(
        false
      );
    });
    ...
    it("should validate with enum constraints", () => {
      ...
      expect(generator.validate(invalidValue, { enum: enumValues })).toBe(
        false
      );
    });
